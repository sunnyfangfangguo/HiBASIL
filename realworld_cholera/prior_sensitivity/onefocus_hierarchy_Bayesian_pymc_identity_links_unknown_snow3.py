...
def create_multi_row_zoib_model(obs_data, model_type='exponential'):
    ...
    with pm.Model() as model:
        ...
        fx1 = pm.Normal("fx1", mu=529396.5394, sigma=250)
        fy1 = pm.Normal("fy1", mu=181025.063, sigma=250)
        ...
        if model_type == 'exponential':
            scale1 = pm.Gamma("scale1", alpha=3, beta=0.015)  
            ...
            scale1 = pm.Gamma("scale1", alpha=3, beta=0.015) 
            exponent1 = pm.Gamma("exponent1", alpha=10, beta=5) 
        ...
        if model_type == 'exponential':
            kernel = fz1 * pt.exp(-distance / pt.maximum(scale1, 0.01))
            
        elif model_type == 'gaussian':
            kernel = fz1 * pt.exp(-(distance**2) / pt.maximum(2 * scale1**2, 0.01))
            
        elif model_type == 'power_law':
            kernel = fz1 / pt.pow(1 + distance / pt.maximum(scale1, 0.01), exponent1)
        else:
...
def plot_multi_row_results(trace, obs_data, focus_coords, y_pred, y_low, y_high, model_type):
    ...
    def predict_row(x, y):
        ...
        if model_type == 'exponential':
            kernel = fz1_est * np.exp(-distance / scale1_est)
        
        elif model_type == 'gaussian':
            kernel = fz1_est * np.exp(-(distance**2) / (2 * scale1_est**2))
            
        elif model_type == 'power_law':
            kernel = fz1_est / np.power(1 + distance / scale1_est, exponent1_est)
    ...
    # Mark foci
    ax.axvline(focus_coords[1], color='black', linestyle='--', alpha=0.7, 
              label=f'Observed Focus (Y={focus_coords[1]})')
    ...
    ax_ins.scatter(focus_coords[0], focus_coords[1], marker='x', color='black', s=40, label='True Focus', zorder=5)
    ax_ins.text(focus_coords[0]+2, focus_coords[1]+5, 'True', fontsize=7, color='black', fontweight='bold')
...
if __name__ == "__main__":
    ...
    focus_coords = [529396.5, 181025.1]
    print("\nDataset summary:")
    ...
    for model_type in models_to_test:
        ...
        if len(traces) > 1:
            ...
            comp_df.to_csv(output_dir + f"models_performances.csv")
