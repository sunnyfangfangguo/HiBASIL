...
def create_multi_row_zoib_model(obs_data):
    ...
    with pm.Model() as model:
        ...
        fz1_base = pm.Beta("fz1_base", alpha=1.0, beta=11)  # Base intensity for focus 1
        ...
        phi = pm.Gamma("phi", alpha=2, beta=0.5) # constant in space
        ...
        p_param = pm.Deterministic("p_param", pt.clip(pi0_row, eps, 1-eps))
...
def plot_multi_row_results(trace, obs_data, y_pred, y_low, y_high):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        r2 = r2_score(row_sev, y_pred_row)        
    ...
    plt.tight_layout()
    plt.savefig(output_dir + 'multi_row_results.png', dpi=600, bbox_inches='tight')
    



def plot_3d_rows_subplots(trace, obs_data, y_pred):
    ...
    plt.savefig(output_dir + 'rows_3d_subplots.png', dpi=300, bbox_inches='tight')    
    print("✓ 3D subplots (2x2 grid) created successfully")


def plot_row_comparison_2d(trace, obs_data, y_pred):
    ...
    plt.tight_layout()
    plt.savefig(output_dir + 'row_comparison_2d.png', dpi=300, bbox_inches='tight')
    
    # Print R² summary
    print("\nRow-specific R² values for model:")
...
def plot_posterior_predictive(y_sim, y_obs, distances):
    ...
    legend_handles = [
        ...
        mpatches.Patch(color='blue', alpha=1.0, label='Predicted') # <--- Solid Blue
    ]
    ...
    # Save figure
    plt.savefig('posterior_predictive_check_null_model.png', dpi=600, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    pd.DataFrame(columns=['overall_r2', 'overall_rmse', 'overall_mae', 'overall_corr',
    ...
    with model:
        ...
        trace.to_netcdf(output_dir + 'control_null_trace.nc')
        ...
        ess_df.to_csv(output_dir + "effective_sample_size.csv")  
        ...
        plot_posterior_predictive(y_sim, y_obs, distances)
        ...
        np.savez(output_dir + 'control_null_predictions.npz',
         y_pred=y_pred,
         y_obs=y_obs,
         distances=distances,
         rows=obs_data['Row'].values)
        ...
        metrics = {
                ...
                'overall_corr': np.corrcoef(y_obs, y_pred)[0, 1],
        ...
        print(diagnostics)
        diagnostics.to_csv(output_dir + 'disgnostic.csv')
        summary.to_csv(output_dir + 'trace_summary.csv')
                            
        "create plots if diagnostics worked"        
        print("\nCreating result plots...")
        plot_multi_row_results(trace, obs_data, y_pred, y_low, y_high)
        print("✓ Plots created successfully")
        
        # Add surface plots with row-specific R²
        plot_surfaces(trace, obs_data, y_pred)
