...
def load_and_process_multi_row_data(df):
    ...
    for row_id in obs_data['Row'].unique():
        ...
        if len(row_foci) == 2:
            ...
            focus2 = row_foci_sorted.iloc[1]
            
            focus_coords[row_id] = {
                'focus1': (focus1['InterrowDistance'], focus1['Distance'], focus1['Severity']),
                'focus2': (focus2['InterrowDistance'], focus2['Distance'], focus2['Severity'])
            }
            
            print(f"Row {row_id}: Focus1=({focus1['InterrowDistance']}, {focus1['Distance']}, focus1['Severity']),"
                  f"Focus1=({focus2['InterrowDistance']}, {focus2['Distance']}, focus2['Severity'])")
...
def create_multi_row_zoib_model(obs_data, model_type='exponential'):
    ...
    with pm.Model() as model:
        ...
        fx1 = pm.Normal("fx1", mu=0.0, sigma=5.0, shape=n_rows)
        fy1 = pm.Normal("fy1", mu=8.0, sigma=10.0, shape=n_rows)
        
        # weakly informed priors for focus2 coordinates based on observed data
        fx2 = pm.Normal("fx2", mu=0.0, sigma=5.0, shape=n_rows)
        fy2 = pm.Normal("fy2", mu=18.0, sigma=10.0, shape=n_rows)
        
        # Shared dispersal parameters (same across all rows)
        fz1_base = pm.Beta("fz1_base", alpha=1, beta=11)  # Base intensity for focus 1
        fz2_base = pm.Beta("fz2_base", alpha=1, beta=11)  # Base intensity for focus 1
        ...
        fz1 = pm.Deterministic("fz1", pt.clip(fz1_base + fz1_row_effect, 0.001, 0.999))

        fz2_row_effect = pm.Normal("fz2_row_effect", mu=0, sigma=sigma_row, shape=n_rows)      
        fz2 = pm.Deterministic("fz2", pt.clip(fz2_base + fz2_row_effect, 0.001, 0.999))
        ...
        if model_type == 'exponential':
            scale1 = pm.Gamma("scale1", alpha=3, beta=0.5)
            scale2 = pm.Gamma("scale2", alpha=3, beta=0.5)
            ...
            exponent1 = pm.Gamma("exponent1", alpha=4, beta=2.0) # !ensure mean > 2
            exponent2 = pm.Gamma("exponent2", alpha=4, beta=2.0) # !ensure mean > 2
         
        # ZOIB mixture parameters (shared across rows)
        q_param = pm.Beta("q_param", alpha=1, beta=50)  # Probability of exact 1 given not 0, constant in space
        pi0 = pm.Beta("pi0", alpha=1, beta=1, shape=n_rows)
        pi0_row = pi0[row_idx]
        ...
        p_param = pm.Deterministic("p_param", pt.clip(pi0_row * (1 - kernel), eps, 1-eps))
...
def plot_multi_row_results(trace, obs_data, y_pred,  y_low, y_high, model_type):
    ...
    fig, axes = plt.subplots(1, 5, figsize=(37.5, 6))
    axes = axes.flatten()
    ...
    for idx, row in enumerate(unique_rows):
        ...
        fz2_row_est = summary.loc[f'fz2[{idx}]', 'mean']
        fx2_row_est = summary.loc[f'fx2[{idx}]', 'mean']
        fy2_row_est = summary.loc[f'fy2[{idx}]', 'mean']
        
        # Create prediction function for this row
        def predict_row(x, y):
            distance1 = np.sqrt((x - fx1_row_est)**2 + (y - fy1_row_est)**2)
            distance2 = np.sqrt((x - fx2_row_est)**2 + (y - fy2_row_est)**2)
            
            if model_type == 'exponential':
                kernel1 = fz1_row_est * np.exp(-distance1 / scale1_est)
                kernel2 = fz2_row_est * np.exp(-distance2 / scale2_est)
            
            elif model_type == 'gaussian':
                kernel1 = fz1_row_est * np.exp(-(distance1**2) / (2 * scale1_est**2))
                kernel2 = fz2_row_est * np.exp(-(distance2**2) / (2 * scale2_est**2))
                
            elif model_type == 'power_law':
                kernel1 = fz1_row_est / np.power(1 + distance1 / scale1_est, exponent1_est)
                kernel2 = fz2_row_est / np.power(1 + distance2 / scale2_est, exponent2_est)
        ...
        ax.axvline(fy2_row_est, color='green', linestyle='--', alpha=0.7, 
                  label=f'Estimated Focus (Y={fy2_row_est})')
        
        ax.set_title(f'Replicate {idx+1}')
        ax.set_xlabel('Distance (y)')
        ax.set_ylabel('Infection Severity')
        ax.legend(loc='upper left', bbox_to_anchor=(0.05, 0.88), fontsize=10)
        ...
        ax_ins.scatter(fx1_row_est, fy1_row_est, marker='*', color='cyan', s=50, label='Estimated')
        ax_ins.text(fx1_row_est + 4, fy1_row_est, 'Estimated', color='darkcyan', 
            fontsize=7, fontweight='bold', va='center')
        
        ax_ins.scatter(fx2_row_est, fy2_row_est, marker='*', color='cyan', s=50, label='Estimated')
        ax_ins.text(fx2_row_est + 4, fy2_row_est, 'Estimated', color='darkcyan', 
            fontsize=7, fontweight='bold', va='center')
...
def plot_3d_rows_subplots(trace, obs_data, y_pred, model_type):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        focus2_pred = predict_row(fx2_row_est, fy2_row_est)   
        
        ax.scatter([fx2_row_est], [fy2_row_est], [focus2_pred], 
                  marker='*', s=150, color='red', 
                  edgecolors='darkred', linewidth=2, label='Focus 1')
        ax.plot([fx2_row_est, fx2_row_est], 
               [fy2_row_est, fy2_row_est], 
        ...
        ax.set_title(f'Row {row}: R² = {r2_row:.3f}', fontsize=12)
...
def plot_row_comparison_2d(trace, obs_data, y_pred, model_type):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        ax.scatter([fx2_row_est], [fy2_row_est], 
                  marker='*', s=200, color='red', 
                  edgecolors='darkred', linewidth=2, zorder=6, label='Focus 1')
...
def plot_posterior_predictive(y_sim, y_obs, distances, model_type):
    ...
    ax1.text(-0.05, 1.05, 'I', transform=ax1.transAxes,
                fontsize=16, fontweight='bold', va='top')    
    ...
    ax2.text(-0.05, 1.05, 'J', transform=ax2.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    ax3.text(-0.05, 1.05, 'K', transform=ax3.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    if mask_cont.sum() > 5:
        ...
        ax4.text(-0.05, 1.05, 'L', transform=ax4.transAxes,
                    fontsize=16, fontweight='bold', va='top')
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('disease_severity_of_year_2023_3rd_survey_control.csv')
    ...
    for model_type in models_to_test:
        ...
        model = create_multi_row_zoib_model(obs_data, model_type=model_type)            
        with model:
            ...
            trace.to_netcdf(output_dir + f'{model_type}_trace_control_twofoci.nc')
