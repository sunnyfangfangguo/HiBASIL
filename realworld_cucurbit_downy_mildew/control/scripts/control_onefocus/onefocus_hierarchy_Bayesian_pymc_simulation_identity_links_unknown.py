...
def create_multi_row_zoib_model(obs_data, model_type='exponential'):
    ...
    with pm.Model() as model:
        ...
        fx1 = pm.Normal("fx1", mu=0, sigma=5.0, shape=n_rows)
        fy1 = pm.Normal("fy1", mu=13.0, sigma=10.0, shape=n_rows)
        ...
        if model_type == 'exponential':
            ...
            scale1 = pm.Gamma("scale1", alpha=3, beta=0.5) 
            exponent1 = pm.Gamma("exponent1", alpha=4, beta=2.0) 
...
def plot_multi_row_results(trace, obs_data, y_pred, y_low, y_high, model_type):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        ax.legend(loc='upper left', bbox_to_anchor=(0.01, 0.9), fontsize=9, frameon=True)
        ...
        ax_ins.text(fx1_row_est + 2, fy1_row_est, 'Estimated', fontsize=7, color='darkcyan', fontweight='bold')
...
if __name__ == "__main__":
    ...
    for model_type in models_to_test:
        ...
        with model:
            ...
            trace.to_netcdf(output_dir + f'{model_type}_trace_control_onefocus.nc')
