...
def create_multi_row_zoib_model(obs_data, focus_coords, model_type='exponential'):
    ...
    focus2_coords = np.zeros((n_rows, 3))
    ...
    for idx, row in enumerate(unique_rows):
        ...
        focus2_coords[idx] = [coords['focus2'][0], coords['focus2'][1], coords['focus2'][2]]
    ...
    with pm.Model() as model:
        ...
        fx2 = pt.as_tensor_variable(focus2_coords[:, 0])  # Shape: (n_rows,)
        fy2 = pt.as_tensor_variable(focus2_coords[:, 1])
...
def plot_multi_row_results(trace, obs_data, focus_coords, y_pred, y_low, y_high, model_type):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        fx2_row = focus2_coords[0]
        fy2_row = focus2_coords[1]
        ...
        def predict_row(x, y):
            distance1 = np.sqrt((x - fx1_row)**2 + (y - fy1_row)**2)
            distance2 = np.sqrt((x - fx2_row)**2 + (y - fy2_row)**2)
            
            if model_type == 'exponential':
                kernel1 = fz1_row * np.exp(-distance1 / scale1_est)
                kernel2 = fz2_row * np.exp(-distance2 / scale2_est)
            
            elif model_type == 'gaussian':
                kernel1 = fz1_row * np.exp(-(distance1**2) / (2 * scale1_est**2))
                kernel2 = fz2_row * np.exp(-(distance2**2) / (2 * scale2_est**2))
                
            elif model_type == 'power_law':
                kernel1 = fz1_row / np.power(1 + distance1 / scale1_est, exponent1_est)
                kernel2 = fz2_row / np.power(1 + distance2 / scale2_est, exponent2_est)
...
def plot_3d_rows_subplots(trace, obs_data, focus_coords, y_pred, model_type):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        focus2_pred = predict_row(fx2_row, fy2_row)   
        
        ax.scatter([fx2_row], [fy2_row], [focus2_pred], 
                  marker='*', s=150, color='red', 
                  edgecolors='darkred', linewidth=2, label='Focus 1')
        ax.plot([fx2_row, fx2_row], 
               [fy2_row, fy2_row], 
...
def plot_posterior_predictive(y_sim, y_obs, distances, model_type):
    ...
    ax1.text(-0.05, 1.05, 'E', transform=ax1.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    ax2.text(-0.05, 1.05, 'F', transform=ax2.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    ax3.text(-0.05, 1.05, 'G', transform=ax3.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    if mask_cont.sum() > 5:
        ...
        ax4.text(-0.05, 1.05, 'H', transform=ax4.transAxes,
                    fontsize=16, fontweight='bold', va='top')
