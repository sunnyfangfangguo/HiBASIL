...
Partial pooling the rows to share scale and exponent dispersal parameters
"""
...
def create_multi_row_zoib_model(obs_data, model_type='exponential'):
    ...
    with pm.Model() as model:
        ...
        "partial pooling rows to share scale and exponent dispersal parameters"
        # log(scale1) ~ Normal(log(scale_mu), scale_sigma)
        # log(exponnet1) ~ Normal(log(exponent_mu), exponent_sigma)
        
        # population-level parameters-scale
        scale_mu = pm.Gamma('scale_mu', alpha=2, beta=2)
        scale_sigma = pm.HalfNormal('scale_sigma', sigma=1.2)
        scale1_raw = pm.Normal('scale1_raw', mu=0, sigma=1, shape=n_rows)
               
        # row-specfici parameters
        if model_type == 'exponential':
            scale1 = pm.Deterministic('scale1', scale_mu * pt.exp(scale_sigma * scale1_raw)) 
            ...
            scale1 = pm.Deterministic('scale1', scale_mu * pt.exp(scale_sigma * scale1_raw))
            exponent1 = pm.Gamma("exponent1", alpha=10, beta=2.0)
        ...
        sigma_row = pm.HalfNormal("sigma_row", sigma=0.3)
        ...
        distance = pt.sqrt((X - fx1)**2 + (Y - fy1)**2)
        
        "Kernel functions with shared parameters"
        if model_type == 'exponential':
            kernel = fz1_row * pt.exp(-distance / pt.maximum(scale1[row_idx], 0.01))
            
        elif model_type == 'gaussian':
            kernel = fz1_row * pt.exp(-(distance**2) / pt.maximum(2 * scale1[row_idx]**2, 0.01))
            
        elif model_type == 'power_law':
            kernel = fz1_row / pt.pow(1 + distance / pt.maximum(scale1[row_idx], 0.01), exponent1)
        else:
...
def plot_multi_row_results(trace, obs_data, focus_coords, y_pred, y_low, y_high, model_type):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        scale1_est_row = summary.loc[f'scale1[{idx}]', 'mean']    
        if model_type == "power_law":
            exponent1_est = summary.loc['exponent1', 'mean']
        
        # Create prediction function for this row
        def predict_row(x, y):
            distance = np.sqrt((x - fx1_est)**2 + (y - fy1_est)**2)
            
            if model_type == 'exponential':
                kernel = fz1_row * np.exp(-distance / scale1_est_row)
            
            elif model_type == 'gaussian':
                kernel = fz1_row * np.exp(-(distance**2) / (2 * scale1_est_row**2))
                
            elif model_type == 'power_law':
                kernel = fz1_row / np.power(1 + distance / scale1_est_row, exponent1_est)
        ...
        ax.axvline(fy1_est, color='green', linestyle='--', alpha=0.7, 
                  label=f'Estimated Focus (Y={fy1_est})')
        ...
        ax_ins.text(fx1_est + 2, fy1_est, 'Estimated', fontsize=7, color='darkcyan', fontweight='bold')
        
        ax_ins.scatter(focus1_coords[0], focus1_coords[1], marker='x', color='black', s=40, label='True Focus', zorder=5)
        ax_ins.text(focus1_coords[0]+2, focus1_coords[1]+5, 'True', fontsize=7, color='black', fontweight='bold')
...
def plot_3d_rows_subplots(trace, obs_data, focus_coords, y_pred, model_type):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        scale1_est_row = summary.loc[f'scale1[{idx}]', 'mean']    
        if model_type == "power_law":
            exponent1_est = summary.loc[f'exponent1', 'mean']
