    ...
    for row_id in obs_data['Row'].unique():
        ...
        if len(row_foci) == 2:
            ...
            print(f"Row {row_id}: Focus1=({focus1['InterrowDistance']}, {focus1['Distance']}, {focus1['Severity']:.1f}), "
                  f"Focus2=({focus2['InterrowDistance']}, {focus2['Distance']}, {focus2['Severity']:.1f})")
...
def create_multi_row_zoib_model(obs_data, model_type='exponential'):
    ...
    with pm.Model() as model:
        ...
        fx1 = pm.Normal("fx1", mu=0.0, sigma=3.0)
        fx2 = pm.Normal("fx2", mu=0.0, sigma=3.0)
        
        fy1 = pm.Normal("fy1", mu=0.0, sigma=10.0)
        fy2 = pm.Normal("fy2", mu=25.0, sigma=10.0)
                
        # Shared dispersal parameters (same across all rows)
        fz1_base = pm.Beta("fz1_base", alpha=2, beta=2)  # Base intensity for focus 1
        fz2_base = pm.Beta("fz2_base", alpha=2, beta=2)  # Base intensity for focus 2
        
        if model_type == 'exponential':
            scale1 = pm.Gamma("scale1", alpha=6, beta=2)
            scale2 = pm.Gamma("scale2", alpha=6, beta=2)
            ...
            exponent1 = pm.Gamma("exponent1", alpha=12, beta=2.0) # !ensure mean > 2
            exponent2 = pm.Gamma("exponent2", alpha=12, beta=2.0) # !ensure mean > 2

         
        # Row-specific random effects on intensity
        sigma_row = pm.HalfNormal("sigma_row", sigma=0.25)
        ...
        pi0 = pm.Beta("pi0", alpha=1, beta=10)
        
        # Precision parameter for Beta distribution
        phi = pm.Gamma("phi", alpha=4, beta=2)
...
def convergence_diagnostics(trace):
    """Extract convergence diagnostics using ArviZ"""
    
    # ArviZ's built-in summary
    summary = az.summary(trace, hdi_prob=0.95)
    
    # Extract max R-hat from summary
    ...
    diagnostics = pd.DataFrame({
        ...
        'rhat_converged': [max_rhat <= 1.01],
        'divergence_acceptable': [n_divergent < 10]
    })
    ...
    print(f"Divergences: {n_divergent} {'✓' if n_divergent < 10 else '✗'}")
    print(f"Parameters: {len(summary)}")
    
    # Show worst parameters
    worst_params = summary.nlargest(5, 'r_hat')[['mean', 'sd', 'r_hat', 'ess_bulk']]
    print("\nWorst R-hat parameters:")
    print(worst_params)
...
def plot_multi_row_results(trace, obs_data, focus_coords, y_pred, y_low, y_high, model_type):
    ...
    fx1_row_est = summary.loc['fx1', 'mean']
    fy1_row_est = summary.loc['fy1', 'mean']
    
    fx2_row_est = summary.loc['fx2', 'mean']
    fy2_row_est = summary.loc['fy2', 'mean']
    ...
    for idx, row in enumerate(unique_rows):
        ...
        focus2_coords = focus_coords[row]['focus2']
        ...
        ax.axvline(focus2_coords[1], color='black', linestyle='--', alpha=0.7, 
                  label=f'Observed Focus (Y={focus2_coords[1]})')
        ...
        ax_ins = inset_axes(ax, width="30%", height="30%", loc='upper right', bbox_to_anchor=(0, -0.05, 1, 1), 
...
def model_performance(traces, obs_data, y_pred):
    ...
    for model_name, trace in traces.items():
        ...
        for idx, row in enumerate(unique_rows):
            ...
            fz2_row = summary.loc[f'fz2[{idx}]', 'mean']
...
def plot_posterior_predictive(y_sim, y_obs, distances, model_type):
    ...
    ax1.text(-0.05, 1.05, 'M', transform=ax1.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    ax2.text(-0.05, 1.05, 'N', transform=ax2.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    ax3.text(-0.05, 1.05, 'O', transform=ax3.transAxes,
                fontsize=16, fontweight='bold', va='top')
    ...
    if mask_cont.sum() > 5:
        ...
        ax4.text(-0.05, 1.05, 'P', transform=ax4.transAxes,
                    fontsize=16, fontweight='bold', va='top')
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('disease_severity_of_year_2023_3rd_survey_twofoci.csv')
    df = quality_control_check(df)
    
    select_rows = [3, 10, 13, 20]
