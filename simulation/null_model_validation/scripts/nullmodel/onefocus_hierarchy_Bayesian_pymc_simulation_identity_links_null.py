...
def zoib_random_fn(p_param, q_param, alpha_vals, beta_vals, rng=None, size=None):
    ...
    if size is not None:
        ...
        output_shape = np.broadcast(p_param, q_param, alpha_vals, beta_vals).shape
    
    n_obs = np.prod(output_shape)
       
    # Broadcast scalar parameters to match output shape
    p_param = np.broadcast_to(p_param, output_shape)
    q_param = np.broadcast_to(q_param, output_shape)
    alpha_vals = np.broadcast_to(alpha_vals, output_shape)
    beta_vals = np.broadcast_to(beta_vals, output_shape)
...
def create_multi_row_zoib_model(obs_data, focus_coords):
    """
...
def plot_multi_row_results(trace, obs_data, focus_coords, y_pred, sim):
...
def plot_3d_rows_subplots(trace, obs_data, focus_coords, y_pred, sim):
...
def plot_row_comparison_2d(trace, obs_data, focus_coords, y_pred, sim):
...
def plot_surfaces(trace, obs_data, focus_coords, y_pred, sim):
    ...
    plot_row_comparison_2d(trace, obs_data, focus_coords, y_pred, sim)
    
    # Create comprehensive 3D plot of all rows
    print("Creating 3D plot of all rows...")
    plot_3d_rows_subplots(trace, obs_data, focus_coords, y_pred, sim)
...
def plot_posterior_predictive(y_sim, y_obs, distances, sim):
    """
    ...
    plt.suptitle(f'Model - Posterior Predictive Check (Sim {sim})',
                 fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    
    # Save
    filename = f'posterior_predictive_check_sim{sim}.png'
...
if __name__ == "__main__":
    ...
    for sim in range(n_sims):
        ...
        null_model = create_multi_row_zoib_model(obs_data, focus_coords)            
        with null_model:
            initial_point = null_model.initial_point()
            print("✓ Initial point computed successfully")
            
            logp_fn = null_model.compile_logp()
            ...
            "store trace to dict"                        
            traces['null_model'] = trace
            
            exponential_trace = az.from_netcdf(f'../output/exponential_trace_sim{sim}.nc')
            gaussian_trace = az.from_netcdf(f'../output/gaussian_trace_sim{sim}.nc')
            power_law_trace = az.from_netcdf(f'../output/power_law_trace_sim{sim}.nc')
            
            traces['exponential'] = exponential_trace
            traces['gaussian'] = gaussian_trace
            traces['power_law'] = power_law_trace
                                    
            "save trace file for later plotting"
            trace.to_netcdf(output_dir + f'null_model_trace_sim{sim}.nc')
            ...
            plot_posterior_predictive(y_sim, y_obs, distances, sim)
            ...
            print(diagnostics)
            diagnostics.to_csv(output_dir + f'null_model_disgnostic_sim{sim}.csv')
            summary.to_csv(output_dir + f'null_model_trace_summary_sim{sim}.csv')
                                
            "create plots if diagnostics worked"                
            print("\nCreating result plots...")
            plot_multi_row_results(trace, obs_data, focus_coords, y_pred, sim)
            print("✓ Plots created successfully")
            
            # Add surface plots with row-specific R²
            plot_surfaces(trace, obs_data, focus_coords, y_pred, sim)
            ...
            ess_df.to_csv(output_dir + f"null_model_effective_sample_size_sim{sim}.csv")                        
