...
def load_and_process_multi_row_data(df, fz1_value=0.2):
...
def zoib_random_fn(p_param, q_param, alpha_vals, beta_vals, rng=None, size=None):
    ...
    # Broadcast scalar parameters to match output shape
    if p_param.size == 1:  # Scalar
        p_param = np.full(output_shape, p_param.item())
    if q_param.size == 1:  # Scalar
        q_param = np.full(output_shape, q_param.item())
...
def create_multi_row_zoib_model(obs_data, focus_coords, model_type='exponential'):
    """
    Create ZOIB model using all rows for control (no spatial dependence)
    """
    eps = 1e-12
    # Prepare data arrays
    ...
    with pm.Model() as model:
        ...
        fz1 = pt.as_tensor_variable(focus1_coords[:, 2])
        ...
        # -------------------------------------------------------------------
        # Define Dispersal Parameters with Priors
        # This makes the test a formal "Prior Influence Control"
        # -------------------------------------------------------------------
        if model_type == 'power_law':
            # Wide priors are set for sampling, but they will be unconstrained by the data
            scale1 = pm.HalfNormal("scale1", 20.0)
            # TruncatedNormal ensures the exponent stays positive and > 1 for the power law
            exponent1 = pm.Gamma("exponent1", alpha=4, beta=2)
        
        elif model_type in ['exponential', 'gaussian']:
            # Define scale for exponential/gaussian (uses HalfNormal)
            scale1 = pm.HalfNormal("scale1", 20.0)
        # -------------------------------------------------------------------
        ...
        "for control, severity is constant has no relationship with distance"
        mu = pm.Deterministic("mu", pt.clip(fz1_row, eps, 1-eps))    
        p_param = pm.Deterministic("p_param", pt.clip(pi0, eps, 1-eps))
...
def plot_multi_row_results(trace, obs_data, focus_coords, y_pred, model_type, sim):
    ...
    for idx, row in enumerate(unique_rows):
        ...
        def predict_row(x, y):
            kernel = np.full_like(x, fz1_row, dtype=float)
        ...
        ax.set_title(f'Row {row}: X={focus1_coords[0]:.1f} (fz1={fz1_row:.3f})')
...
def plot_3d_rows_subplots(trace, obs_data, focus_coords, y_pred, model_type, sim):
...
def plot_surfaces(trace, obs_data, focus_coords, y_pred, model_type, sim):
    ...
    plot_row_comparison_2d(trace, obs_data, focus_coords, y_pred, model_type, sim)
    
    # Create comprehensive 3D plot of all rows
    print("Creating 3D plot of all rows...")
    plot_3d_rows_subplots(trace, obs_data, focus_coords, y_pred, model_type, sim)
...
def plot_prior_posterior_control(trace, prior, model_type, sim):
    """
    Plots the prior and posterior density for the dispersal parameters 
    using a manual KDE plot, compatible with older ArviZ versions.
    """
    
    # Define parameters to plot based on model type
    param_names = ['scale1']
    param_labels = [r'Scale ($\alpha$)']
    
    if model_type == 'power_law':
        param_names.append('exponent1')
        param_labels.append(r'Exponent ($\kappa$)')
    ...
    for i, name in enumerate(param_names):
        ...
        # 2. Extract and plot Prior
        # The prior samples are stored under the .prior group in the prior object
        prior_samples = prior.prior[name].values.flatten()
        az.plot_kde(
            prior_samples, 
            ax=ax, 
            label='Prior', 
            plot_kwargs={'color': 'black', 'linestyle': '--', 'linewidth': 2}
        )
        
        ax.set_title(f'{param_labels[i]} - Prior Influence Control')
        ax.set_xlabel('Value')
        ax.set_ylabel('Density')
        ax.legend()
        
    fig.suptitle('Prior vs. Posterior Density: Successful Control Test (Overlapping Distributions)', fontsize=14, y=1.02)
    plt.tight_layout()
    # You may want to add a savefig call here
    fig.savefig(f'contro_test_prior_posterior_density_{model_type}_{sim}.png')



def calculate_prior_posterior_metrics(trace, prior, model_type, sim):
    """
    Calculate metrics comparing prior and posterior for one simulation.
    Returns dict of metrics.
    """
    metrics = {'simulation':sim, 'model_type': model_type}
    ...
    for param in param_names:
        ...
        prior_std = prior_samples.std()
        post_mean = posterior_samples.mean()
        post_std = posterior_samples.std()
        
        # Key metrics for control test
        mean_shift = post_mean - prior_mean  # Should be near 0
        relative_shift = mean_shift / prior_mean  # As percentage
        std_ratio = post_std / prior_std  # Should be near 1
        
        # Overlap (simple version)
        # What % of posterior mass is within prior 95% CI?
        prior_lower = np.percentile(prior_samples, 2.5)
        prior_upper = np.percentile(prior_samples, 97.5)
        overlap = np.mean((posterior_samples >= prior_lower) & 
                         (posterior_samples <= prior_upper))
        
        # Store
        metrics[f'{param}_prior_mean'] = prior_mean
        metrics[f'{param}_prior_std'] = prior_std
        metrics[f'{param}_post_mean'] = post_mean
        metrics[f'{param}_post_std'] = post_std
        metrics[f'{param}_mean_shift'] = mean_shift
        metrics[f'{param}_relative_shift'] = relative_shift
        metrics[f'{param}_std_ratio'] = std_ratio
        metrics[f'{param}_overlap'] = overlap
...
if __name__ == "__main__":
    ...
    fz1_obs = 0.2  # Focus 1 intensity for each observation
    ...
    y_coords = np.unique(np.concatenate([[0.0], y_coords]))
    ...
    "for control, p_param is constant and has no relationship with distance"
    # calculate zero-inflation probability
    p_param = np.clip([p0] * len(sample_points), eps, 1- eps)
    
    "for control, severity is constant has no relationship with distance"
    mu = np.clip([fz1_obs] * len(sample_points), eps, 1- eps)
    ...
    for sim in range(n_sims):
        ...
        for model_type in models_to_test:
            ...
            with model:
                ...
                # step_scale = pm.Metropolis(vars=[model.scale1])
                # if model_type == 'power_law':
                #     step_exponent = pm.Metropolis(vars=[model.exponent1])
                #     steps = [step_scale, step_exponent]
                # else:
                #     steps = [step_scale]
                ...
                trace.to_netcdf(output_dir + f'{model_type}_trace_sim{sim}.nc')
                ...
                plot_multi_row_results(trace, obs_data, focus_coords, y_pred, model_type, sim)
                print("✓ Plots created successfully")
                
                # Add surface plots with row-specific R²
                plot_surfaces(trace, obs_data, focus_coords, y_pred, model_type, sim)
                ...
                prior = pm.sample_prior_predictive(samples=1000, random_seed=42+sim)
                plot_prior_posterior_control(trace, prior, model_type, sim)
                
                prior_post_metrics = calculate_prior_posterior_metrics(trace, prior, model_type, sim)
                
                # Save to CSV (append mode)
                metrics_file = output_dir + 'prior_posterior_metrics.csv'
                pd.DataFrame([prior_post_metrics]).to_csv(
                    metrics_file, 
                    mode='a', 
