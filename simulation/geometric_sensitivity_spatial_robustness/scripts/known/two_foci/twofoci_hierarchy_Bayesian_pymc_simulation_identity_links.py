...
def load_and_process_multi_row_data(df, fz1_value=0.3, fz2_value=0.4):
    ...
    for row_id in obs_data['Row'].unique():
        ...
        if len(row_foci) == 2:
            ...
            focus_coords[row_id] = {
                ...
                'focus2': (focus2['InterrowDistance'], focus2['Distance'], fz2_value)
            }
            
            print(f"Row {row_id}: Focus1=({focus1['InterrowDistance']}, {focus1['Distance']}, fz1_value),"
                  f"Focus1=({focus2['InterrowDistance']}, {focus2['Distance']}, fz2_value)")
...
def create_multi_row_zoib_model(obs_data, focus_coords, model_type='exponential'):
    ...
    with pm.Model() as model:
        ...
        fz2 = pt.as_tensor_variable(focus2_coords[:, 2])
...
if __name__ == "__main__":
    ...
    y_coords = np.linspace(start=-2, stop=52, num=250) # 10 points from Y=0 to Y=12
    ...
    for sim in range(n_sims):
        ...
        obs_data, focus_coords = load_and_process_multi_row_data(df, fz1_value=fz1_obs, fz2_value=fz2_obs)
        print("\nDataset summary:")
