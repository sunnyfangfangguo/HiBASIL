...
def extract_metrics(df, sim, model_type, true_values_dict):
    ...
    "***********fx1 bias to true second foci************"
    fx2_rows = df[df.index.str.startswith('fx1[', na=False)]    # because the one focu model only estimate one focus
    ...
    "***************fy1 bias to true second foci*******************"
    fy2_rows = df[df.index.str.startswith('fy1[', na=False)]    
    ...
    "***************fz1 bias to true second foci*******************"
    fz2_rows = df[df.index.str.startswith('fz1[', na=False)]    
