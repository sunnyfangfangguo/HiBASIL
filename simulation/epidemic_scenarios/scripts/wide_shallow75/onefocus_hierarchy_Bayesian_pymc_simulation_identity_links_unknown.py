...
def create_multi_row_zoib_model(obs_data, model_type='exponential'):
    ...
    with pm.Model() as model:
        ...
        if model_type == 'exponential':
            scale1 = pm.Gamma("scale1", alpha=5, beta=0.33)  
            ...
            scale1 = pm.Gamma("scale1", alpha=5, beta=0.33) 
            exponent1 = pm.Gamma("exponent1", alpha=3, beta=2.0) # !ensure mean > 2
...
if __name__ == "__main__":
    ...
    y_coords = np.linspace(start=-2, stop=75, num=250) # 10 points from Y=0 to Y=12
